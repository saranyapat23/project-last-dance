...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $targetPath)) {
            ...
            $error = '⚠️ ไม่สามารถอัปโหลดรูปภาพได้';
        }
    }
  ...
  <form class="details-card" method="POST" action="" enctype="multipart/form-data">
    <div class="row align-items-center">
      <div class="menu-options">
        ...
        <label class="btn btn-outline-secondary" for="dessert">DESSERT</label>
      </div>

      <!-- รูป -->
      <div class="col-md-4 text-center">
        <img id="preview" src="" alt="รูปเมนู" style="max-width:300px; margin-top:15px; border-radius:15px;"
    ...
    <div class="button-wrapper" style="margin-top: 50px;">
      <button class="btn cancel-new" type="button" onclick="window.location.href='../../backoffice/edit/showmenu.php'">
        <p class="btn-p">Cancel</p>
      </button>
      <button class="btn confirm-new" type="submit">
        <p class="btn-p">Confirm</p>
      </button>
    </div>
  </form>
</div>
